...
export function Button({ href, children, light=false }: { href:string; children:ReactNode; light?:boolean }) {
  const isBooking = typeof children === "string" && (children.toLowerCase().includes("book") || href.includes("book-consultation"));
  return (
    <Link className={`btn ${light ? "btn-light" : "btn-primary"}`} href={href}>
      {isBooking ? <CalendarDays size={16} /> : null}
      <span>{children}</span>
      {!isBooking ? <ArrowRight size={15}/> : null}
    </Link>
  );
}
